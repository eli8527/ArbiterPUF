...
public class CardCom {
	...
	private static int SIZE = 64 ; // Challenge size, in bits
...
//				 System.out.println("\n");
	...
	private static void sendDelays(String delayFile) {
		...
		try {
			...
			int idx = 5;
			byte[] c1 = new byte[5 + 32]; // Zero Delay value APDU Call
			...
			c0[1] = (byte) 0x02;
			c1[1] = (byte) 0x02; // Instruction

			// P1 - challenge size
			c0[2] = (byte) 0x00; // zero delay
			c1[2] = (byte) 0x01; // one delay
			...
			c1[4] = (byte) new Integer(32).byteValue();
			...
			while (input != null) {
				...
				short zeroVal = Short.parseShort(vals[0]);
				short oneVal = Short.parseShort(vals[1]);

				// Shorts to bytes
				byte[] tmp0 = shortToByteArray(zeroVal);
				byte[] tmp1 = shortToByteArray(oneVal);
...
//					}
//					System.out.println();
					byte[] send1 = sendReceiveDelays(c1);
...
//					System.out.println();
			...
			System.out.println("Delay Transmission Successful!");
		} catch (IOException e) {
			System.out.println(e);
		}
	}
